...
//float readFunctionModBus(const uint8_t &func, const uint8_t &addr, const uint16_t &reg, uint8_t &count = 1, const bool &isFloat = 0)
float readFunctionModBus(const uint8_t &func, const uint8_t &addr, const uint16_t &reg, uint8_t &count , const bool &isFloat)
{
  float retValue = 0;
  if (_modbusUART)
  {
...
class ModbusMaster1 : public IoTItem
{
private:
...
public:
  ModbusMaster1(String parameters) : IoTItem(parameters)
  {
...
bool state = param[2].valD;
      if (_debug)
        {
          SerialPrint("I", "ModbusMaster", "readInputRegisters, addr: " + String((uint8_t)_addr, HEX) + ", regStr: " + _regStr + ", reg: " + String(_reg, HEX) + ", state: " + String(state) + " = result: " + String(result, HEX));
        }
      }
      return val;
    }
    ...
    else if (command == "readCoils") // vout = mb.readCoils(1, \"0х0000\", 1) - "Адрес","Регистр","Кличество бит"
    {
      if (param.size())
      {
        ...
        val.valD = readFunctionModBus(0x01, _addr, _reg, count, isFloat);
      }
      return val;
    }
    else if (command == "readDiscreteInputs") // vout = mb.readDiscreteInputs(1, \"0х0000\", 1) - "Адрес","Регистр","Кличество бит"
    {
      if (param.size())
      {
        ...
        _reg = hexStringToUint16(param[1].valS);
        node.begin(_addr, (Stream &)*_modbusUART);
        val.valD = readFunctionModBus(0x02, _addr, _reg, count, isFloat);
      }
      return val;
    }
...
void *getAPI_ModbusRTU(String subtype, String param)
{
  ...
  {
    return new ModbusMaster1(param);
  }
  {
    return nullptr;
  }
