...
const String endpoint = "https://weather.rambler.ru/api/v3/map_towns/?url_path=v-kishinyove";
//const String endpoint = "https://weather.rambler.ru/api/v3/today/?all_data=0&url_path=v-kishinyove";
//const String endpoint = "http://api.openweathermap.org/data/2.5/weather?q=Lisbon,pt&APPID=";//https://weather.rambler.ru/api/v3/today/?all_data=0&url_path=v-kishinyove
//const String key = "yourAPIkey";

String jsonBuffer;//===================
String line; //============================
class ramWeather : public IoTItem
{
private:
...
public:
    ramWeather(String parameters) : Weatherdoc1(1024), IoTItem(parameters)
    {
    ...
    void getWeather()
    {
        ...
        if (isNetworkActive())
        {
            ...
            {
                // Для нового API
                 //  urlReq = "https://weather.rambler.ru/api/v3/map_towns/?url_path=v-kishinyove";
            ...
            https.begin(client, urlReq); // urlCurrent
...
//   HttpClient http;
    ...
    https.begin(endpoint); //Specify the URL
    //http.begin(endpoint + key); //Specify the URL
    int httpsCode = https.GET();  //Make the request
...
/*
            ...
            {
                ret = https.errorToString(httpCode).c_str();
            }
            */
            if (_debug)
            {
                SerialPrint("<-", F("getRamblerWeather"), urlReq);
                SerialPrint("->", F("getRamblerWeather"), "server: " + ret);
            }
            https.end();
        }
...
//   HttpClient http;
    ...
    https.end(); //Free the resources
                                          ...
                                          /// отправка в Serial
  Serial.println();  
  String name = Weatherdoc1["current_town"]["name"];                           // достаем имя, 
  Serial.print("name:");
  Serial.println(name);  
  
  float temp = Weatherdoc1["weather"]["temp"];                   // достаем температуру из структуры main
  //float tempC = tempK - 273.15;                         // переводим кельвины в цельсий
  Serial.print("temp: ");
  Serial.print(temp);                                  // отправляем значение в сериал
  Serial.println(" C");


        getWeather();
        if (jsonReadStr(Weatherdoc1["weather"], "temp", true) != "null")
        {
            ...
            {
                value.valS = jsonReadStr(Weatherdoc1["weather"], _param, true);
                regEvent(value.valS, "ramWeather");
            }
            else if (_param == "pressure")
            {
                ...
                regEvent(tval / 1.333, "ramWeather");
            }
...
void *getAPI_ramWeather(String subtype, String param)
{
    if (subtype == F("ramWeather"))
    {
        return new ramWeather(param);
    }
    else
    {
        return nullptr;
